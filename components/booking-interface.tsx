...
export default function BookingInterface() {
  ...
  return (
    <div className="bg-white/90 w-full p-5 sm:p-6 flex-grow lg:rounded-2xl lg:border lg:border-gray-200 lg:shadow-xl lg:flex-grow-0">
      ...
      <div className="space-y-4 mb-4">
        {/* Pickup Date & Time - side-by-side on all screens */}
  ...
  disabled={(date) => {
    ...
    // base: today or pickup date, whichever is set
    let minBase = today;
    if (pickupDate) {
      const p = new Date(pickupDate);
      p.setHours(0, 0, 0, 0); // strip time so whole day is allowed [web:29][web:33]
      minBase = p;
    }
    ...
    // disable only strictly earlier calendar days
    return d < minBase;
  }}
  initialFocus
/>
